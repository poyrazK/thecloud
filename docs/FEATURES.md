...
- **Automated Failover**: A dedicated `DatabaseFailoverWorker` performs periodic TCP health checks on primary instances. If a primary becomes unreachable, the worker queries replication status on all linked replicas (via `pg_stat_replication` for PostgreSQL) and promotes the replica with the lowest replication lag (≤ 5 seconds threshold).
