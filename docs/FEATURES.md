...
- **Automated Failover**: A dedicated `DatabaseFailoverWorker` performs periodic TCP health checks on primary instances. If a primary becomes unreachable, the worker queries replication lag on all linked replicas (using `pg_last_xact_replay_timestamp()` for PostgreSQL standbys) and promotes the replica with the lowest replication lag (≤ 5 seconds threshold).
