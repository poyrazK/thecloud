...
func TestRBACService_IAMIntegration(t *testing.T) {
	...
	t.Run("AllowByPolicy", func(t *testing.T) {
		userRepo.On("GetByID", ctx, userID).Return(&domain.User{ID: userID, Role: "viewer"}, nil).Once()
		...
		iamRepo.On("GetPoliciesForUser", ctx, userID).Return(policies, nil).Once()
	...
	t.Run("DenyByPolicyOverridesRole", func(t *testing.T) {
		userRepo.On("GetByID", ctx, userID).Return(&domain.User{ID: userID, Role: "admin"}, nil).Once()
		policies := []*domain.Policy{
			{
				Statements: []domain.Statement{
	...
	t.Run("FallbackToRole", func(t *testing.T) {
		...
		userRepo.On("GetByID", ctx, userID).Return(&domain.User{ID: userID, Role: "custom-dev"}, nil).Once()
		iamRepo.On("GetPoliciesForUser", ctx, userID).Return([]*domain.Policy{}, nil).Once()
