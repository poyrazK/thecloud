...
func TestIAMEvaluator_Evaluate_Allow(t *testing.T) {
	...
	effect, err := evaluator.Evaluate(ctx, policies, "instance:launch", "arn:aws:ec2:us-east-1:123456789012:instance/i-1234567890abcdef0", nil)
...
func TestIAMEvaluator_Evaluate_WildcardAction(t *testing.T) {
	...
	for _, tt := range tests {
		effect, err := evaluator.Evaluate(ctx, policies, tt.action, tt.resource, nil)
		require.NoError(t, err)
...
func TestIAMEvaluator_Evaluate_WildcardResource(t *testing.T) {
	...
	for _, tt := range tests {
		effect, err := evaluator.Evaluate(ctx, policies, "instance:launch", tt.resource, nil)
		require.NoError(t, err)
		assert.Equal(t, tt.expected, effect, "resource %s should match", tt.resource)
	}
}