...
func setupInstanceServiceTest(t *testing.T) (*pgxpool.Pool, *services.InstanceService, coreports.ComputeBackend, coreports.InstanceRepository, coreports.VpcRepository, coreports.VolumeRepository, context.Context) {
	...
	backendType := os.Getenv("COMPUTE_BACKEND")
	if backendType == "libvirt" {
		// Use libvirt adapter if requested
		compute, err = libvirt.NewLibvirtAdapter(slog.Default(), "")
		if err != nil {
			t.Logf("Warning: failed to initialize libvirt adapter, falling back to docker: %v", err)
	...
	if compute.Type() == "docker" {
		dCompute := compute.(*docker.DockerAdapter)
		_, _ = dCompute.CreateNetwork(ctx, testutil.TestDockerNetwork)
		// Pre-pull test image to prevent flakes in CI environments with slow registries or restrictive daemons
		_, _, _ = dCompute.LaunchInstanceWithOptions(ctx, coreports.CreateInstanceOptions{
			Name:      "pre-pull-image",
			ImageName: testImage,
		})
		_ = dCompute.DeleteInstance(ctx, "pre-pull-image")
	}
