...
func setupClusterServiceTest(t *testing.T) (*MockClusterRepo, *MockClusterProvisioner, *MockVpcService, *MockInstanceService, *MockTaskQueue, *MockSecretService, *MockRBACService, ports.ClusterService) {
	t.Helper()
...
func TestClusterServiceGetKubeconfigAdmin(t *testing.T) {
	t.Parallel()
	repo, _, _, _, _, secretSvc, _, svc := setupClusterServiceTest(t)
	userID := uuid.New()
	tenantID := uuid.New()
	ctx := appcontext.WithTenantID(appcontext.WithUserID(context.Background(), userID), tenantID)
	clusterID := uuid.New()
...
func TestClusterServiceGetKubeconfigNonAdmin(t *testing.T) {
	t.Parallel()
	repo, provisioner, _, _, _, _, _, svc := setupClusterServiceTest(t)
	userID := uuid.New()
	tenantID := uuid.New()
	ctx := appcontext.WithTenantID(appcontext.WithUserID(context.Background(), userID), tenantID)
	clusterID := uuid.New()
